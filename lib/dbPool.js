...
class DBPool {
  ...
  async retireAll() {
    ...
    for ( let i = 0; i < me.connections.length; i++){
      try {
        await me.retire(me.connections[i]);
        //since retire removes one element from connections the length is not the same
        //need to the adjust the index back one
        i--;
      } catch (error) {
  ...
  /**
   * Shorthand to exec a statemnt , just provide the sql to run.
  ...
  async runSql(sql = '') {
    let me = this;
    let conn = me.attach(), result = [];
    try {
      me.log(`Executing SQL...\nSQL Statement: ${sql}`);
      result = await conn.getStatement().exec(sql);
      me.log(`Executed SQL`);
    } catch (err) {
      me.log(`SQL Error: ${err}`);
      await me.retire(conn);
    }
    try {
      await me.detach(conn);
    } catch (error) {
      await me.retire(conn);
  ...
  async prepareExecute(sql = '', params = []) {
    let me = this, conn = me.attach(), stmt = conn.getStatement(),
      resultSet = [], outParams = [];
    ...
    try {
      ...
      if (params.length > 0){
        ...
        await stmt.bindParam(me.formatParams(params));
      }
      outParams = await stmt.execute();
    } catch (err) {
      me.log(` SQL Error: ${err}`);
      me.retire(conn);
    }
  ...
  formatParams(params) {
    ...
    for ( let p of params  ){
      if (typeof p === 'string'){
        boundParams.push([p, idbp.SQL_PARAM_INPUT_OUTPUT, idbp.SQL_BIND_CHAR]);
      } else if (typeof p === 'number') {
        boundParams.push( [p, idbp.SQL_PARAM_INPUT_OUTPUT, idbp.SQL_BIND_NUMERIC] );
      } else if (typeof p === null) {
        boundParams.push([p, idbp.SQL_PARAM_INPUT_OUTPUT, idbp.SQL_BIND_NULL_DATA]);
      } else {
        this.log(`Param that caused error was ${JSON.stringify(p)}`);
