...
class Statement {
   ...
   * @memberof Statement
   */
  async bindParam(params, options) {
    const { stmt } = this;
    let parameters = params; // This is done to avoid eslint no-param-reassign

    return new Promise((resolve, reject) => {
      if (options && typeof options === 'object' && options.setupParams) {
        parameters = setupParams(params);
      }
      stmt.bindParam(parameters, (error, result) => {
        if (error) {
   ...
   * @memberof Statement
   */
  async bind(params, options) {
    await this.bindParam(params, options);
  }
