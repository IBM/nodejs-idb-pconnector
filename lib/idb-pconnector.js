...
class Connection {
	 ...
	 * @param {string} [dbname] - the name of the database to connect to. If a name is not specified, the dbname is defaulted to "*LOCAL".
	 ...
	 * @example Available Set & Get Connection Attributes:
		...
		Further Documentaion {@link https://www.ibm.com/support/knowledgecenter/en/ssw_ibm_i_73/cli/rzadpfnsconx.htm HERE}
	 * @memberof Connection
	 */
	async getConnAttr(attribute) {
		...
		return new Promise(function (resolve, reject) {
			try {
				resolve(wrp.getConnAttr(attribute));
			} catch (dbError) {
				reject(dbError);
			}
		});
	...
	async setConnAttr(attribute, value) {
		...
		return new Promise(function (resolve, reject) {
			try {
				resolve(wrp.setConnAttr(attribute, value));
			} catch (dbError) {
				reject(dbError);
			}
		});
 ...
 * @class Statement
 * @constructor
 * @param {Object} [dbConn] - a Connection Object to intialize the Statement , if a connection was not provided one will be intialized for the statement.
 */
class Statement {
	constructor(dbConn) {
	    if(dbConn == undefined) {
			this.dbc = new Connection().connect().wrp;
			this.stmt = dba.dbstmt(this.dbc);
		} else {
	 ...
	 * @param {Array} params - this should be an Array of the parameter list. Each parameter element will also be an Array with 3 values ( Value, In/out Type ,Indicator ).
	...
	async fetch() {
		var stmt = this.stmt;
		return new Promise(function (resolve, reject) {
			stmt.fetchSync(function (result, dbError) {
				if (dbError == undefined)
					resolve(result);
				reject(dbError);
			});
	...
	async fetchAll() {
		var stmt = this.stmt;
		return new Promise(function (resolve, reject) {
			stmt.fetchAllSync(function (result, dbError) {
				if (dbError == undefined)
					resolve(result);
				reject(dbError);
			});
	...
	async fieldName(index) {
		...
		return new Promise(function (resolve, reject) {
			if (index === undefined || !isInt(index))
				reject("Must pass an int index as a parameter");
			try {
				resolve(stmt.fieldName(index));
			} catch (dbError) {
				reject(dbError);
			}
		});
	...
	async fieldNullable(index) {
		...
		return new Promise(function (resolve, reject) {
			if (index === undefined || !isInt(index))
				reject("Must pass an int index as a parameter");
			try {
				var res = stmt.fieldNullable(index);
				res = makeBool(res);
	...
	async fieldPrecise(index) {
		...
		return new Promise(function (resolve, reject) {
			if (index === undefined || !isInt(index))
				reject("Must pass an int index as a parameter");
			try {
				resolve(stmt.fieldPrecise(index));
			} catch (dbError) {
				reject(dbError);
			}
		});
	 ...
	 * @example Available Set & Get Statement Attributes
		...
		Further Documentaion {@link https://www.ibm.com/support/knowledgecenter/en/ssw_ibm_i_73/cli/rzadpfnsstma.htm HERE}
	 ...
	 * @param {number} attribute - musbt be an int INT.
	 ...
	 * @param {number} recno - indicates which error should be retrieved. The first error record is number 1.
	 * @returns {Promise} - Promise object represents Number retrieved from the execution of stmtError(). 
	 * @memberof Statement
	 */
	async stmtError(hType, recno) {
		var stmt = this.stmt;
		return new Promise(function (resolve, reject) {
			stmt.stmtError(hType, recno, function (result, dbError) {
				if (dbError == undefined)
					resolve(result);
				reject(dbError);
			});
 ...
 * @param {any} data - the input data to be checked before returning.
